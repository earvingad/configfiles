...
"LEFT_MASTER")
			...
			X=$((LEFT_MARGIN +USELESS_GAPS/2)) 
			Y=$((TOP_MARGIN+USELESS_GAPS/2))
			#set the width to the default MASTER_WIDTH 
			W=$((MASTER_WIDTH -USELESS_GAPS))
			H=$(( HEIGHT -TOP_MARGIN - BOTTOM_MARGIN -TITLE_BAR_HEIGHT))
			...
			wmctrl	-r :ACTIVE: -e "0,$X,$Y,$W,$H"
			 ...
			 W=$((WIDTH - MASTER_WIDTH -USELESS_GAPS -USELESS_GAPS))
			 #the height would be equally shared by the rest of the windows
			 H=$(((H-USELESS_GAPS)/(WINDOWS_IN_DESK - 1) - TITLE_BAR_HEIGHT ))
			 i=0
			  while  [ "$i"  -le  "$WINDOWS_IN_DESK" ] ;  do
				...
				#set the attributes 
					...
					Y=$((Y+H+TITLE_BAR_HEIGHT+USELESS_GAPS/2))
				fi
...
"GRID")
			...
			H=$(((HEIGHT-TOP_MARGIN-BOTTOM_MARGIN-USELESS_GAPS)/NUMBER_OF_ROWS - TITLE_BAR_HEIGHT))
				...
				if  [[ "$CURRENT_ROW" -lt "$NUMBER_OF_ROWS" ]]; then
					if [[ "$CURRENT_ROW_WINDOWS" -eq "NORMAL_ROW_WINDOWS " ]]; then 
						...
						if [[ "$CURRENT_ROW" -eq "$NUMBER_OF_ROWS" ]] ; then 
							X=$((LEFT_MARGIN +USELESS_GAPS))
							Y=$((Y+H+TITLE_BAR_HEIGHT+USELESS_GAPS))
							W=$((BOTTOM_ROW_WIDTH+USELESS_GAPS/2))
						else
							CURRENT_ROW_WINDOWS=0
						fi	
					fi
					if [[ "$CURRENT_ROW_WINDOWS" -eq "0" ]] ; then
						X=$((LEFT_MARGIN +USELESS_GAPS))
						W=$((NORMAL_ROW_WIDTH +USELESS_GAPS/2 ))
